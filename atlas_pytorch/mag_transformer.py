...
# sliding window attention for MAG
# simpler than MAC's SegmentedAttention - no segment boundaries, pure sliding window
...
class MemoryAsGateTransformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        for layer in layers:
            ...
            # neural memory (if this layer uses it)
            mem = None
            gate = None
            ...
            self.layers.append(ModuleList([attn, mem, gate, ff]))
    ...
    def forward(
        ...
        for layer_idx, (attn, mem, gate, ff) in enumerate(self.layers):
            ...
            # memory branch (if exists)
            if exists(mem):
                # memory operates on same input, with state propagation
                mem_out, next_mem_state = mem(x, state = mem_state)
                next_mem_states.append(next_mem_state)
                
                # gated combination: o = attn_out + gate * mem_out
                g = gate(mem_out)
